class ParameterBag {
    ...
    keySeparator := "."
    tokenReplacer := ""
    ; ParameterBag works like a map, but doesn't extend Map, so this allows the list cloner and merging to treat it like a map.
    isMapLike := true
    ; ParameterBag deep-clones itself inside its Clone() method, so instruct the list cloner to do a shallow clone only.
    ...
    Clone() {
        cloned := super.Clone()
        cloned.parameters := List.Clone(cloned.parameters, true)
        cloned.Context := List.Clone(cloned.Context, true)
    ...
    Merge(key, value, deep := true) {
        ...
        if (this.Has(key)) {
            value := List.Merge(this.Get(key), value, deep)
        }
